...
public class pre_enq_with_extend_report {
...
//	Precedence Wise Execution:
	...
	public void beforeMethod() {
		...
		spark = new ExtentSparkReporter("/home/aditya/Documents/eclipse-workspace/AUTOMATION_PROJECT/Extent_reports");
		extent = new ExtentReports();
		extent.attachReporter(spark);
		
		
		spark.config().setTimeStampFormat("EEEE, MMMM dd, yyyy, hh:mm a '('zzz')'");
		spark.config().setReportName("Sai_Roshini_Report_upto_GetScore");
		spark.config().setDocumentTitle("SaiRoshini_Java_Selenium_TestNG_ExtentReport");
	...
	public void pre_enq_upto_GetScore() throws InterruptedException, AWTException {
				...
				test = extent.createTest("Pre_enq_Upto_Send_Enq", "1");
...
//				success message after fetch kyc
				try {
					...
					if (sucess_message == true) {
						test.log(Status.INFO, "KYC fetched sucessfully---"+" ["+failed+"]");
...
//				scroll to cibil score
		        ...
		        if(sucess_msg== true) {
		        	...
		        	test.log(Status.INFO, "Download Cibil processed---["+fetch_msg+"]");
		        ...
		        test.log(Status.INFO, "Value of CIBIL Score is---["+get_cibil+"]");
...
//				validating Get score
		        ...
		        if(sucess_msg_of_get_score== true) {
		        	...
		        	test.log(Status.INFO, "Get Score Sucess Message---"+" ["+validate_get_score+"]");
		        }
		        else {
...
//		        CIBIL Limit Value
	        	...
	        	test.log(Status.INFO, "CIBIL Limit Value---"+" ["+validate_CIBIL_Limit_Value+"]");
...
//	        	Final Decision
	        	...
	        	test.log(Status.INFO, "Final Decision---"+" ["+validate_Final_Decision+"]");
...
//				Scroll to bslink
		        ...
		        test.log(Status.INFO, "bs link url is---"+" ["+bs_link_url_value+"]");
...
//		        search bank
		       ...
		       test.log(Status.INFO, "Account Statement Uploaded ");
...
//				Click on Get score
				...
				if( Download_bank_done_score == true) {
					...
					test.log(Status.INFO, "Get Score Clicked");
				}
				else {
...
//				Scroll to avg_balance_limit
		        ...
		        test.log(Status.INFO, "Value of avg balance---"+value_ofavg_balance );
...
//		        Manual final value
		        ...
		        test.log(Status.INFO, "Manual_final_Decision_value---"+Manual_final_Decision_value );
...
//				Avaliable Credit Limit
		        ...
		        test.log(Status.INFO, "Value_of_updated_credit_limit---"+Value_of_updated_credit_limit );
...
//		        final_decision
		        ...
		        test.log(Status.INFO, "Value_of_updated_final_decision---"+Value_of_updated_final_decision );
...
//		        click_on_bank_validation
				...
				if( bank_validation == true) {
					...
					test.log(Status.INFO, "Bank Validation Sucessful");
				}
				else {
...
//		        click_on_pre aproved
				...
				if( Pre_Aproved == true) {
					...
					test.log(Status.INFO, "Pre Aproved Completed");
				}
				else {
...
//				Send_to_enq
				...
				if( Send_to_enq == true) {
					...
					test.log(Status.INFO, "Send to enquiry Sucessful");
				}
				else {
	...
	public void delete_process_enquiry() throws InterruptedException {
			...
			test = extent.createTest("delete_process_enquiry","2");
